...
class WordIndex:
    ...
    def build_from_line(cls, line_data, synset_map):
        ...
        synsets = [
            synset_map[int(line_split[i], 10)]
            for i in range(6 + ptr_count, 6 + ptr_count + synset_count)
        ]
    ...
    def __repr__(self):
        return "%s%s" % (
            ...
            (self.lemma, self.category, self.ptrs, self.synsets, self.tagsense_count),
        )
...
class WordIndexDictFormatter(WordIndex):
    ...
    def dict_str(self):
        tw = TextWrapper(
            width=self.LINE_WIDTH_MAX,
            initial_indent=(
                self.prefix_fmtf_line_first % self.category_map_rev[self.category]
            ),
...
class Synset:
    ...
    def build_from_line(cls, line_data):
        ...
        ptrs = [
            (line_split[i], line_split[i + 1], line_split[i + 2], line_split[i + 3])
        ...
        if tok != "|":
            ...
            frames = [
                (int(line_split[i + 1], 10), int(line_split[i + 2], 16))
                for i in range(base, base + frame_count * 3, 3)
            ]
    ...
    def __repr__(self):
        return "%s%s" % (
            self.__class__.__name__,
            (self.offset, self.type, self.words, self.ptrs, self.gloss, self.frames),
        )
...
%(wn_license)s"""
    ...
    def dict_entry_write(cls, file_index, file_data, key, entry, linesep="\n"):
        ...
        file_index.write(
            ...
            % (
                key,
                cls.base64_encode(entry_start),
                cls.base64_encode(entry_len),
                linesep,
            )
        )
    ...
    def dict_generate(self, file_index, file_data):
        ...
        file_data.write("\n")
        self.dict_entry_write(
            file_index, file_data, "00-database-allchars", "00-database-allchars\n"
        )
        fmt_vars = {
            "wn_url": self.wn_url,
...
if __name__ == "__main__":
    ...
    op = optparse.OptionParser(
        usage="usage: %prog [options] (<wn_index_file> <wn_data_file>)+"
    )
    op.add_option(
        "-i",
        "--outindex",
        dest="oi",
        default="wn.index",
        help="filename of index file to write to",
    )
    op.add_option(
        "-d",
        "--outdata",
        dest="od",
        default="wn.dict",
        help="filename of data file to write to",
    )
    op.add_option(
        "--wn_url",
        dest="wn_url",
        default="ftp://ftp.cogsci.princeton.edu/pub/wordnet/2.0",
        help="URL for wordnet sources",
    )
    op.add_option(
        "--db_desc_short",
        dest="desc_short",
        default="     WordNet (r) 2.1 (2005)",
        help="short dict DB description",
    )
    op.add_option(
        "--db_desc_long",
        dest="desc_long",
        default="    WordNet (r): A Lexical Database for English from the\n     Cognitive Science Laboratory at Princeton University",
        help="long dict DB description",
    )

    (options, args) = op.parse_args()

    wnd = WordnetDict(
        wn_url=options.wn_url,
        desc_short=options.desc_short,
        desc_long=options.desc_long,
    )
    ...
    print(
        "All input files parsed. Writing output to index file %r and data file %r."
        % (options.oi, options.od)
    )
